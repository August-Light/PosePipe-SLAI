...
def onmouse(event, x, y, flags, param):
    ...
    in the current window, and other features. If the mouse clicked down, then it starts selecting and stops
    tracking. If the mouse lifts up, then it stops selecting and sets tracking to True. If in between, and the mouse moved
    ...
    if event == cv2.EVENT_LBUTTONDOWN:   # If left mouse button started to be pressed
        ...
        isTracking = False
        isDragging = True
    elif event == cv2.EVENT_LBUTTONUP:  # left mouse button up indicates end of selecting
        isDragging = False
        drag_start = None
        if selection != None:
            isTracking = True
    elif isDragging and event == cv2.EVENT_MOUSEMOVE:    # if currently selecting and mouse is moving
...
cv2.moveWindow('histogram', 700, 100)   # Move to reduce overlap
...
while True:
    ...
    if isDragging and selection != None:    # if currently selecting and a good region has been selected
